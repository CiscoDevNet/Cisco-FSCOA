...
cmd_subnet_mgmt='aws ec2 create-subnet --vpc-id' + " " + "{}".format(vpcid) + " " + '--region' + " " + "{}".format(region) + " " + '--availability-zone' + " " + "{}".format(az) + " " + '--cidr-block 10.100.2.0/24 --tag-specifications' + " " "'ResourceType=subnet,Tags=[{Key=Name,Value=SUBNET_mgmt}]'"
...
cmd_subnet_cluster='aws ec2 create-subnet --vpc-id' + " " + "{}".format(vpcid) + " " + '--region' + " " + "{}".format(region) + " " + '--availability-zone' + " " + "{}".format(az) + " " + '--cidr-block 10.100.3.0/24 --tag-specifications' + " " "'ResourceType=subnet,Tags=[{Key=Name,Value=SUBNET_cluster}]'"
...
#12 - ASSOCIATE THE mgmt subnet WITH THE DEFAULT VPC ROUTE TABLE
outfile_ass_mgmt_sub = 'ass_rt_mgmt_sub.json'
ass_rt_mgmt_sub='aws ec2 associate-route-table --region' + " " + "{}".format(region) + " " + '--route-table-id' + " " + "{}".format(def_vpc_rt_id) + " " + '--subnet-id' + " " + "{}".format(subnetid_mgmt)
...
print("ASSOCIATE The public subnet TABLE WITH THE DEFAULT VPC ROUTE TABLE")
outfile_ass_public_rt_pub_sub = 'ass_public_rt_pub_sub.json'
ass_public_rt_pub_sub='aws ec2 associate-route-table --region' + " " + "{}".format(region) + " " + '--route-table-id' + " " + "{}".format(def_vpc_rt_id) + " " + '--subnet-id' + " " + "{}".format(subnetid_public)
print(ass_public_rt_pub_sub)
output = check_output("{}".format(ass_public_rt_pub_sub), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
with open(outfile_ass_public_rt_pub_sub, 'w') as my_file:
    my_file.write(output)


#asociate the cluster subnet with the default route table
#subnetid_cluster
print("ASSOCIATE The cluster subnet TABLE WITH THE DEFAULT VPC ROUTE TABLE")
outfile_ass_public_rt_cluster_sub = 'ass_public_rt_cluster_sub.json'
ass_public_rt_cluster_sub='aws ec2 associate-route-table --region' + " " + "{}".format(region) + " " + '--route-table-id' + " " + "{}".format(def_vpc_rt_id) + " " + '--subnet-id' + " " + "{}".format(subnetid_cluster)
print(ass_public_rt_cluster_sub)
output = check_output("{}".format(ass_public_rt_cluster_sub), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
with open(outfile_ass_public_rt_cluster_sub, 'w') as my_file:
    my_file.write(output)

#Create a Security Group for both private and public subnets and set inbound traffic allow
priv_sg_name='priv_sg'
out_file_sg_SDWAN='outfile-sg-private.json'
cmd_priv_security_group='aws ec2 create-security-group --group-name --region' + " " + "{}".format(region) + " " + " " + "{}".format(priv_sg_name) + " " + '--description' + " " + "{}".format(priv_sg_name) + " " + '--vpc-id' + " " + "{}".format(vpcid)
output = check_output("{}".format(cmd_priv_security_group), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
...
with open (out_file_sg_SDWAN) as access_json:
    ...
    SDWAN_priv_sg_id=question_access
    SDWAN_priv_sg_id_var=('SDWAN_priv_sg_id=' + "'" + "{}".format(SDWAN_priv_sg_id) + "'")
    print(SDWAN_priv_sg_id_var)
with open(outfile_vars, 'a+') as my_file:
    my_file.write(SDWAN_priv_sg_id_var + "\n")

#Create inbound rule on the security group to all traffic
#aws ec2 authorize-security-group-ingress --group-id sg-1234567890abcdef0 --protocol tcp --port 22 --cidr 0.0.0.0/0
auth_inbound_ssh='aws ec2 authorize-security-group-ingress --region' + " " + "{}".format(region) + " " + '--group-id' + " " + "{}".format(SDWAN_priv_sg_id) + " " + '--protocol -1 --cidr 0.0.0.0/0'
output = check_output("{}".format(auth_inbound_ssh), shell=True).decode().strip()
print("Output: \n{}\n".format(output))

#Create a Security Group for public and set it to allow all traffic from the priv
...
print("Create inbound rule on the security group to allow SSH")
#aws ec2 authorize-security-group-ingress --group-id sg-1234567890abcdef0 --protocol tcp --port 22 --cidr 0.0.0.0/0
...
#Create inbound rule on the default security group to allow All traffic from the priv securtity group for the private subnet
#aws ec2 authorize-security-group-ingress --group-id sg-0702770aa9f0d76ef --ip-permissions '[{"IpProtocol": "-1", "UserIdGroupPairs": [{"GroupId": "sg-0c42d0dc2a8158c4d"}]}]'
auth_inbound_ssh='aws ec2 authorize-security-group-ingress --region' + " " + "{}".format(region) + " " + '--group-id' + " " + "{}".format(SDWAN_sg_id) + " " + '--ip-permissions' + " " + "'" + '''[{"IpProtocol": "-1", "UserIdGroupPairs": [{"GroupId":"''' + "{}".format(SDWAN_priv_sg_id) + '"' + "}]}]'"
print(auth_inbound_ssh)
output = check_output("{}".format(auth_inbound_ssh), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
...
print("Create inbound rule on the security group to allow 443")
#aws ec2 authorize-security-group-ingress --group-id sg-1234567890abcdef0 --protocol tcp --port 443 --cidr 0.0.0.0/0
auth_inbound_ssl='aws ec2 authorize-security-group-ingress --region' + " " + "{}".format(region) + " " + '--group-id' + " " "{}".format(SDWAN_sg_id) + " " + '--protocol tcp --port 443 --cidr 0.0.0.0/0'
print(auth_inbound_ssl)
output = check_output("{}".format(auth_inbound_ssl), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
...
#write SDWAN_priv_sg_id to vault

url = "http://prod-vault.devops-ontap.com:8200/v1/concourse/sdwan/" + name + "/" + "SDWAN_priv_sg_id"
print(url)
...
#data = f'{{"token": "{TOKEN}"}}'
data_json = {"SDWAN_priv_sg_id": SDWAN_priv_sg_id }
